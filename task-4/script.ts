...
const movieGenre = movies.map(({ genre }) => genre).flat().filter((el, index, arr) => arr.indexOf(el) == index)
console.log(movieGenre)

// 2 Собрать в массив всех актеров всех фильмов (без повторения)

const movieActors = movies.map(({ actors }) => actors).flat().filter((el, index, arr) => arr.indexOf(el) == index)
console.log(movieActors)

// 3 Отсортировать фильмы по рейтингу по убыванию

const sortMovie = movies.sort((a, b) => b.imdbRating - a.imdbRating).map(({ imdbRating }) => imdbRating)
...
function getNameFilms2(movie, str) {
...
console.log(getNameFilms2(movies, "Luke Skywalker"))
